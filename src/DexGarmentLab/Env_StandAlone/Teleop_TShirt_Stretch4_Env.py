...
different garment: TNSC_Tshirt4, from DexGarmentLab's OWN Garment/
...
Base scale for this asset is 1.0, not DexGarmentLab's usual 0.0085 --
...
# RCareWorld-2.0 is a sibling directory to DexGarmentLab.
...
# Briefly swapped to RCareWorld-2.0's female1_cp.usd per "마네킹을 여자
# 애셋으로 바꿔줘", reverted back to this original asset per "아니다
# 그냥 이전 사람 마네킹같은애로 바꿀게".
HUMAN_USD = os.path.join(
    os.getcwd(), "Assets", "Human", "Collected_human_model", "biped_demo_meters.usd")
# Per "걔한테 들어가 riro logo는 그냥 지워줘" -- the mannequin's own suit
...
# override right after it per "마네킹 색을 단색으로 표현해줘 예를
# 들면 베이지 색으로", which replaces the texture entirely anyway).
...
# Per "목덜미랑 밑에 단 부분을 특히 크게 만들고 쉽게 마네킹에 끼울 수
# 있도록. 그리고 세로 길이는 짧게" -- widen the collar and hem openings
...
# Per "옷을 좀 더 크게 만들어줘 목이랑 밑단을 더크게 그리고 대신 세로
# 길이는 그대로 유지해줘" -- pushed the overall/neckline/hem scales up
...
TSHIRT_RESHAPE_OVERALL_XZ_SCALE = 1.12  # "옷을 좀 더 크게" -- overall width/depth bump
...
TSHIRT_RESHAPE_VERTICAL_SCALE = 1.0     # "세로 길이는 그대로 유지" -- no shoulder-to-hem compression
...
# the cloth trail the gripper instead of being whipped along with it, per "마우스로
# 직접 티셔츠 잡아서 끌려오는 것처럼 부드럽게".
#
...
# is the "약하게 잡으면 들어 올릴 때 항상 티셔츠가 미끄러져 내려가는" symptom,
...
#
# Raised 0.6 -> 1.0 (real gravity) per "자꾸 바람에 날리네": at 0.6 the cloth
...
# Halved 0.005 -> 0.0025, then set straight to 0.001 per "옷 파티클 무게를
# 0.001로". Briefly suspected (and reverted) as the cause of jittery
# "바람에 날리는 것처럼" footage, but that turned out to be a request to
...
# of them -- which is what "양쪽에서 당겨서 늘렸을 때 계속 빠지는" looks
...
# the default. Briefly moved to 1e2 per "옷이 좀 잘 늘어나게 바꿔줘", put
# back to 1e4, set to 1e3 per "신축성을 1e3으로 해줘" (a midpoint between
# the two measured points above, not itself directly measured by
# scripts/probe_pull.py), then back to 1e4 again per "신축성 다시
# 돌려줘".
...
# Per "정말 확실하게 잡힐때만 잡히고 아니면 못잡게 해줘 -- 어중간하게
# 잡힐때 뭔가 날리는 현상이 심하고 옷이 잘 안따라오는" -- the eligibility
...
# 5 -> 12 per "그리퍼는 확실하게 잡은 경우만 잡히도록" -- requiring more
...
# is rejected outright same as before). 12 -> 25 per "파티클이 12개가
# 아니라 더 높게 설정해줘". Mesh has 566 total points, so 25 is still a
...
# up, which is the reported "약하게 잡으면 들어 올릴 때 항상 티셔츠가 미끄러져
# 내려가는" -- the pinned particles cannot carry the rest of the cloth and the
...
# Per "그리퍼 닫을때 가장 가까운 메쉬들로 붙게하는 기능 넣어줄수 있어?
# 일단 해보고 이상하면 바로 원복할게" -- the strict pincer check above is
...
# Per "일단 그리퍼 닫으면 붙는 코드 이전으로 원복할게 이건 불안정해서
# 아직은 못쓸 듯" -- turned off after live testing. Mechanism kept in
# place (this was built with exactly this toggle for exactly this
# situation), not deleted, per "아직은" -- revisit later if wanted.
ENABLE_GRASP_NEAREST_FALLBACK = False

# Per "마네킹이랑 상호작용이 이상하게 겹치면 옷이랑 스트레치 로봇이
# 그냥 폭발해서 없어지는데, 이거를 방지해줄 수 있어? ... 튕기는 상황
# 전에" -- a severe collision violation (e.g. cloth badly interpenetrating
...
# single launch ("맨 처음부터 멈춰버리네"). Raised to 8.0 -- still too
# sensitive in real interactive use per "방지 기능이 너무 예민한데" (a
...
# "충돌여유 마진 최소한으로", and bend/shear/solver/damping/gravity/
# mass were briefly pushed hard per "완전빵빵하게/거의 강체로" -- all
# reverted back to those original values per "옷 처음 설정으로 돌려줘
# 원본" (scope confirmed: stiffness/weight/margin only -- GRASP_MIN_
...
#
# Briefly cut ALL FOUR to a third (0.015/0.012) together per "옷 충돌여유
# 마진을 좀 줄여줘 그리고 마네킹도 옷 근처만 가도 지금 충돌판정이
# 일어나서 잡기가 어려워", reverted back to 0.045/0.036 per "옷은
# 원복해줘 충돌여유마진".
#
# Per "옷 메쉬 자체들끼리의 충돌여유마진은 높이고 그리퍼랑 마네킹과의
# 충돌여유마진은 얇게 만들어 줄 수 있어? 서로 따로 적용되도록" -- these
...
#     (gripper fingers, mannequin, table). This is the "그리퍼랑
#     마네킹과의" margin -- cut thin.
...
#     the "옷 메쉬 자체들끼리의" margin -- raised instead of cut.
# Never actually needed new plumbing, just different numbers.
#
# Per "옷 자체의 메쉬끼리의 충돌여유마진은 유지하고 옷이 그리퍼랑
# 마네킹과는 충돌여유마진은 좀더 줄여줘" -- GARMENT_PARTICLE_CONTACT_
...
# against the previous 0.015/0.012), then per "좀 더 올려줄 수 있어?"
...
# Per "세밀한 렌더링 다 지우고 최소한의 뻣뻣한 강체적 움직임만" -- was
...
# that only tests it sitting still. Per "너무 강체로 만들면 계속 튀네"
...
# 100. That is not an accident -- they were raised for "최소한의 뻣뻣한 강체적
# 움직임만" and then reverted from 500000 back to 3000 -- but bend stiffness is
# exactly what resists DRAPE, and the request that follows ("윗부분을 잡았는데
# 아랫부분이 중력 때문에 흘러내리는 현상이 별로 없는 것 같아") is the opposite
...
# 6 -> 12 per "설정 강화해줘". Untouched by the "처음 설정으로" revert
# above -- that request scoped to stiffness/weight/margin only.
GARMENT_EDGE_MASS_FACTOR = 12.0

# Per "바닥면이 지금 너무 큰데 줄여줘도 돼 그리고 지금처럼 격자 말고
# ... 무조코 스트레치 환경의 배경 애셋을 참고해서 디자인 업그레이드
# 하고 싶어" -- Real_Ground's own default_environment.usd asset doesn't
...
FLOOR_SIZE = 6.0
# Per "너무 산만해" (the wood grain read as too busy/cluttered) -- swapped
# for a plain grey stone tile texture instead. This is the SAME basecolor
# image DexGarmentLab's own Assets/Scene/ground/tiling.usd already tries
...
# 60% of what it was, per "상자 위치를 조금 낮춰줘 한 60%정도로": 0.95 -> 0.57
...
#   +0.11/side -> 1.2m     what is set here, per "BOX_GAP=1.2를 기본값으로".
#
...
# [isaac-5.1.0 compat: four shirts]
# Pushed away from the figure, per "상자를 좀 멀리 띄우고 싶어 마네킹이랑 떨어지게"
# and then further again per "상자들의 위치를 좀 더 뒤로 ... 스트레치는 그 중간에".
...
#
#   spread  0, elevation   0    0.845   앞으로 나란히, hands furthest forward
...
# per "반대로 해줘" -- both are 180 degree flips (order-independent as a
...
GARMENT_HEM_COLOR = (0.85, 0.05, 0.05)  # 1.4 -> 1.1 -> 1.0, per "옷 크기를 조그만 줄여줘"
...
# Human's own default 0.7, per "쉽게 끼울 수 있도록 좀 작게" (small
...
# scale, feet-to-floor Z offset -0.02588) per "마네킹을 여자 애셋으로
# 바꿔줘", reverted back to these original values per "아니다 그냥 이전
# 사람 마네킹같은애로 바꿀게".
HUMAN_POS = np.array([0.0, 2.5, 0.0])
# Non-uniform, per "마네킹 키를 줄이고 몸통은 키워줘" -- was uniform
...
# for "키를 줄이고", indices 0 and 2 (raw_x/raw_z, the body's width and
# depth) brought UP to 0.60/0.55 for "몸통은 키워줘" -- shorter and
...
#
# Per "마네킹이 너무 작네 키만 좀 더 키워줘 어깨나 머리는 유지하고" --
...
# in pose_arms_at_attention (per "얼굴을 좀 줄이고"), and since that's a
...
#
# Per "마네킹 크기를 좀 더 키워줘" -- this time "크기" (size), not "키"
...
# height-only bump. No "머리는 유지" qualifier was repeated this time,
...
# y: the middle of the lane, per "스트레치는 그 중간에 위치하도록" -- exactly
...
# simulation afterward, this is purely a fixed pose bake matching "강체로
# 진행" (treat as rigid after this). Originally "앞으로 나란히" (arms
# forward, reaching toward the approaching robot); switched to 차렷
...
def pose_arms_at_attention(stage, skel_root_path, arm_dir=None,
    ...
    space. None keeps the original straight-down (차렷) pose.
    ...
    # (0,-1,0) (raw -Y) for 차렷/attention, arms hanging straight down in
    # the scene; briefly flipped to (0,1,0) (raw +Y, straight up) per
    # "양팔을 하늘로 가게 해줘", then reverted back to (0,-1,0)/down per
    # "사람 손 올리지 말고 그냥 차렷자세로 해줘" -- straight down in the
    # Z-up scene again, same as the file's own original attention pose.
    #
    # Per "마네킹팔을 약간 모아줘 손끝이 닿게는 하지말고" -- ARM_CONVERGE_
    # DEG used to tilt each arm slightly INWARD from straight vertical.
    # Per "마네킹 팔이 돌아갔는데... 원본 마네킹 자료 참고해서" -- a
    ...
    # arm's own final axis, added per "손바닥이 서로 마주보도록" (palms
    # facing inward) and mirrored per "팔이 외회전 했어 내회전해야돼".
    # Per "마네킹 팔이 돌아갔는데... 원본 마네킹 자료 참고해서" -- that
    ...
    # Per "마네킹 팔이 돌아갔는데... 원본 마네킹 자료 참고해서" -- even
    ...
    for shoulder_suffix, elbow_suffix, clavicle_suffix in _arm_pairs:
        ...
        # Lengthens the arm per "팔 길이를 10%만 늘려줘": scales the
        ...
        # per "팔길이 20% 줄이고" (the earlier ask was +10%; this is a
        # different, later request to shrink it instead).
        #
        # Girth now moves WITH length, per "둘레가 상박하박 안맞는데":
        ...
        # alone -- only 상박/하박 (upper arm/forearm) was asked about.
        # Reverted to 1.0 (off) per "그냥 팔 복원해줘 너무 이상하다" --
    ...
    # Per "얼굴을 좀 줄이고" -- scales just the Head joint's own local
    ...
    # head_idx = None). Extended per "머리 크기를 조금 줄여줘 90%정도" to also
    ...
    # Shortens the neck per "마네킹 목 길이를 줄여줄 수 있어?": the head
    ...
    # Shrinks the whole upper body per "마네킹 상체를 좀 줄여줄 수 있어?"
    # (length only -- tried length+girth together first, then "아 그냥
    # 원복하고 마네킹 상체 길이만 짧게 만들어줘" dropped the girth half) --
    ...
    # attribute with the result once. Matches "강체로 진행" precisely --
...
class TeleopTShirtStretch4_Env(BaseEnv):
    def __init__(self):
        ...
        # only, no skeletal simulation -- matches "사람은 강체로 진행하면
        # 될 것 같아"). Human.py's own default orientation=[90,0,0]
        ...
        # own comments on that mapping). Currently posed 차렷 (attention,
        ...
        if os.environ.get("STRETCH4_HUMAN", "mesh") == "mesh" and os.path.exists(MESH_HUMAN_USD):
            ...
            pose_arms_at_attention(
                ...
                # [isaac-5.1.0 compat: four shirts]
                # 20 -> 0 -> 10 -> 0 -> 10, per "팔각도를 좌우로 약간
                # 벌렸는데 11자로 앞으로 나란히로 해줘" (parallel, 0) then
                # "팔각도 이전처럼 조금만 벌릴게" (back to the 10-degree-out
                # try, i.e. "이전" = the 10 this same knob held earlier, not
        ...
        # Per "마네킹 색을 단색으로 표현해줘 예를 들면 베이지 색으로" --
        ...
        # Per "마네킹의 충돌 여유 마진을 최대한 줄여줄 수 있어? 지금 옷
        # 틈 사이로 잘 안들어가" -- Human.py turns on collision
        ...
        # Pushed even lower per "마네킹 충돌여유 마진을 최소한으로" --
        ...
        # the skin everywhere, which is the reported "몸에 닿기 전에 티셔츠가 닿는
        # 느낌". The mitten handles the fingers directly, so the body can go back
        ...
        # Stop the two robots colliding with each other, per "로봇끼리는 충돌하지
        # 않도록 해 줄래?" -- the wheels in particular, which is where the user saw
        # them knock together.
        #
...
# Per "스트레치의 디자인을 좀 수정하고 싶어 ... 그 xml파일 형식의 디자인
# 파일 ... 참고해서 로봇 색깔을 입혀줄래?" -- ported from
...
def stretch4_tuning(rig, restore=False):
    ...
    running on the asset's stock stiffness, which is the "P키를 누르고 다시
    시작하니까 조작감이 이상해지더라" the user hit.
    ...
    def _attempt_grab():
        ...
        for ci, gc in enumerate(garment_cloths):
            ...
            if (SINGLE_LAYER_GRASP and len(idx) > GRASP_MIN_PARTICLES
                ...
                # makes it slide out of the fingers on the way up ("약하게
                # 잡으면 들어 올릴 때 항상 티셔츠가 미끄러져 내려가는 듯해").
...
# real gripper cannot do that, and the user reported exactly it: "F3 상태에서는
# 아무것도 관통되지 않는 상태가 맞고, 내가 잡고 직접 옷을 사람 쪽으로 당겨야
# 관통이 일어나".
#
...
#
# "사람 mesh는 천을 뚫고 나오지 못하게 해 버리고 싶은데" -- and with PhysX
...
#
# The requirement that shapes it: "버튼을 누르면 현재 카메라 각도로 녹화가
# 시작되고, 녹화 도중에 카메라 각도 고쳐도 녹화하는 카메라 각도는 유지되도록
# ... P라던지 F1키 등으로 뭔가를 리셋하거나 불러오더라도 녹화는 똑같은 카메라
# 각도로 계속되도록". So:
#
...
def main():
    ...
    # and everything else looked frozen) and now F too ("아이작심 기본
    # 키야?" -- yes: two separate registrations, isaacsim.gui.menu's
...
""")
    ...
    # "방지 기능이 너무 예민한데" -- 6 checks 5 frames apart is ~0.5s of
