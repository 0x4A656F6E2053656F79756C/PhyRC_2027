# Latest: Restored pre-randomization baseline

2026-09-10. The user reported repeated unexpected behavior, including frozen
cloth, and explicitly requested restoration to the version immediately before
randomization. The chosen baseline is **104d6b0**, not the first commit c5d57e5.
Their 16 simulation source files are identical; 104d6b0 additionally retains
installation guidance, the environment-aware doctor and Home-checkout records.

- Maintained checkout: /home/seoyul/PhyRC_2027, branch feat/policy-learning-env.
  A new restoration commit preserves history; no reset/force-push or main merge.
- Previous HEAD 0ba8c25 is preserved on the backup branch
  backup/pre-randomization-rollback-0ba8c25. Later work remains recoverable.
- All tracked baseline files were restored. Only this handoff and new rollback
  documentation/evidence differ from baseline. Do not automatically reapply
  the removed policy adapter, randomization or interval-face patch.
- Human/chair use original fixed placement; the shirt uses the original third
  table. Arm pose/length, native grasp and original contact code are restored.
  RGB-D/policy reset/step/training-demo modules and --no-randomization are absent.
- Runtime was moved aside before preparation. Removed modules and leftover
  bytecode caches were moved out of maintained source and active runtime.
  All 16 source files and prepared copies match 104d6b0 byte for byte.
- GUI was stopped. Old runtime, last GUI log, bytecode caches and copies of
  both existing user slots are in ignored output/rollback-backup-0ba8c25.
  Original slots retained contents and mtimes; smoke used isolated slots.
  Preserve unrelated docs/PhyRC_proposals.pdf and the participant checkout.
- Actual GPU smoke passed: exact original scene, finite cloth, both robots
  moving, gripper toggle and zero-error checkpoint restore. This is not a
  full dressing test or proof of the reported freeze's cause.
- See docs/ROLLBACK.md and docs/verification-results/pre-randomization-rollback.json.
  Existing cache/image can be reused. Extra installed Gymnasium is unused;
  baseline requirements are restored.
...
The historical handoff below describes 104d6b0. Its old main-branch statements
are historical; the checkout/branch and rollback intent above govern.
