...
환경은20Hz에 공개 관측을 제공하며 정책은 두 로봇의 `(2,9)` 명령을 동시에 출력합니다.
...
`load_policy(checkpoint, observation_space, action_space)`가 callable 정책을 반환하도록 Python 파일을 만듭니다.
평가기에서 전달하는 실행 관측은 CSV/HDF5와 달리 원래 shape입니다:
...
출력 shape/범위/유한성 검사를 통과해야 합니다. 모델 출력에 맞는 범위 제한은 참가자 어댑터에서 처리하세요.
...
`--seconds`는 시뮬레이션 에피소드 제한입니다. `--seeds`는 서로 다른2개 이상을 지정합니다.
