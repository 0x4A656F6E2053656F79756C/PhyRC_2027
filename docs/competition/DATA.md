...
용어가 생소하다면 [용어·배열 표기 안내](GLOSSARY.md)를 먼저 읽어보세요.
...
> [!IMPORTANT]
> **ESC는 조종 종료이며, 기본 모드의 학습 파일 생성은 그 이후에도 계속됩니다.**
> Isaac Sim 창이 닫힌 뒤에도 실행한 터미널을 열어 두세요.
> **`[Dataset export] READY ...`가 나오기 전에는 터미널을 닫거나 `Ctrl+C`로 중단하지 마세요.**
> 기록 길이와 PC 성능에 따라 변환 시간이 달라집니다. READY를 확인한 후에 종료하면 됩니다.
> 중간에 중단했다면 아래의 변환 재시도 안내를 확인하세요.
...
조종 중 다시 시도하려면 3D 장면 영역을 클릭하고 **키보드의 `P` 키**를 누릅니다.
장면이 초기화되며 초기화 전후의 시도는 `demo_0`, `demo_1`처럼 별도 에피소드로 저장됩니다.
수집을 모두 마쳤을 때는 ESC로 종료하고 READY까지 기다립니다.

### 재생·검증용 원본 기록

학습 수집 시 함께 생기는 `full_teleop/<실행ID>/`에는 물리 계산 단계마다의 로봇 자세·관절 상태,
옷의 정점 위치·속도, 제어 입력과 파지 상태 등 재생·검증용 정보가 저장됩니다.
이 기록으로 장면을 다시 재생하거나 평가할 수 있습니다. 정책 학습에는 `policy.hdf5`의 허용 관측·액션을 사용합니다.

`./run.sh gui --full-record 1`은 이 원본 상태 기록만 별도로 수집하는 옵션입니다.
카메라 RGB·depth 이미지와 학습용 HDF5는 생성하지 않습니다.
학습 데이터를 수집할 때는 `--training-record 1`이 원본 기록까지 함께 저장하므로 이 옵션을 추가할 필요가 없습니다.
...
**Hz는 시뮬레이션 1초당 처리 횟수**입니다. 20Hz는 0.05초마다 한 번이라는 뜻입니다.
정책 명령 하나를 60Hz 제어 3회, 240Hz 물리 12회 동안 유지합니다.
물리 계산 1회를 tick이라고 부르므로 `end_tick - start_tick = 12`는 action 하나가 끝났다는 뜻입니다.
...
policy.hdf5
  ...
  data/demo_1/ ...           키보드 P로 초기화한 다음 시도
```

**`(N,18)`은 N행 × 18열**입니다. 한 행은 한 시점에서 두 로봇에 보낸 총 18개 명령입니다.
...
보상 0은 학습 파일 형식을 위한 기본값이며 평가 점수와 다릅니다.

기본 구조는 [robomimic 데이터 형식](https://robomimic.github.io/docs/datasets/overview.html)을 따릅니다.
모델별 이미지 전처리와 PhyRC 환경에서 정책을 실행하는 연결 코드(rollout 어댑터)는 필요합니다. 임의 프레임을 학습/검증으로 섞기보다 에피소드 단위로 나누세요.
...
| 필드 | 실행 시 배열 크기(shape) | 펼친 열의 의미 / 단위 |
|---|---|---|
...
| simulation_time_s | scalar(숫자 하나) | 시뮬레이션 시각(s); HDF5/CSV에서는 원소1개 |
