...
새 clone 설치와 빠른 학습은 [참가자 시작 안내](PARTICIPANT_QUICKSTART.md)를 따른다.
...
action[0, 3] = 0.1  # robot_0 lift 속도 0.14 m/s 요청
...
|---|---|---|
| 0 | base 전진 속도 | 0.56m/s |
| 1 | base 왼쪽 속도 | 0.56m/s |
| 2 | base yaw 속도, +Z 기준 반시계 양수 | 2.6rad/s |
| 3 | lift 속도 | 1.4m/s |
| 4 | telescoping arm 전체 신장 속도 | 1.1m/s |
| 5–7 | wrist yaw/pitch/roll 속도 | 각각 5rad/s |
...
`simulation_time_s`는 실제 물리 clock이다. reset이 F1–F5 사용자 파일을 읽거나
덮어쓰지 않는다. seed는 배치를 재현하며 천 초기 복원은 1e-7m 이내인지 검사한다.
...
결과는 [검증 기록](VERIFICATION.md)을 참고한다.
