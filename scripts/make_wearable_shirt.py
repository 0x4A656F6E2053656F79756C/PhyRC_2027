...
# rounded heap with the sleeve tubes folded under it, which is what "소매가 아래
# 방향으로 쳐져있는데 ... 진짜 t모양이 되도록" is about.
#
...
# Shorten how far the sleeves reach from the torso, per "소매길이를 조금만
# 줄여줘". The torso itself (excluding sleeves) runs a roughly constant X-span
...
# at all and the stripe silently vanished ("밑단 줄무늬가 사라졌어"). The
...
# corners out of the band and left the stripe looking centred ("줄이 가운데에만
# 있는데"). Any-vertex-touches-the-band is inclusive enough to keep the full
...
# instead, per "옷의 목구멍을 조금 크게 하고 소매의 구멍을 작게 해줘". No cuff
