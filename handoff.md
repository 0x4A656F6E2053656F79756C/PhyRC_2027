...
다음 에이전트는 이 요약부터 읽고, 필요한 세부 구현·검증 경위만 아래 작업 이력에서 찾아볼 것.
사용자의 현재 요청은 최신 상태를 다음 에이전트가 이어받도록 정리하는 것이며, 앞선 요청 작업은 완료되었다.

## 저장소와 게시 상태

- 작업 경로: `/home/seoyul/PhyRC_2027`.
- GitHub: https://github.com/0x4A656F6E2053656F79756C/PhyRC_2027
- **로컬·원격 모두 main 브랜치 하나만 사용한다.** competition은 main에 모든 이력을 합친 후 삭제했다.
  `docs/competition/`은 참가자 문서 폴더 이름이며, 별도 브랜치가 있다는 뜻이 아니다.
- GitHub 저장소는 public이고 기본 브랜치는 main이다. 사용자에게 “내부용”이라고 불렸던 과거 콘텐츠와 공개 여부를 혼동하지 말 것.
- 최근 기능·문서 변경: `fea7dc7`(용어 안내·새 설치 검증·smoke 수정),
  `d4d2930`(main 단일 대회용 브랜치·조종/기록 설명), `ae1d848`(영상 출처 단정 문구 삭제).
  그 뒤 이 인계 요약을 정리했다. 실제 최신 커밋은 `git log -1`과 원격 상태로 확인할 것.
- 인계 정리 직전 작업 트리는 깨끗했고 main/origin/main은 동일했다.
  이 문서 정리까지 커밋·푸시하며, 대기 중인 구현 작업은 없다.

## 현재 실행과 수집

```bash
./run.sh gui                                      # 조종 연습
./run.sh gui --training-record 1 --evaluate 1      # 학습 데이터 수집 + 자동 평가
./run.sh cpu /scripts/evaluate_replay.py /output/full_teleop/<실행ID> --output /output/evaluation/replay_check
./run.sh replay output/full_teleop/<실행ID> --evaluate /output/evaluation/replay_visual
```

- 3D 화면을 클릭하고 키보드로 조종. **P는 키보드 P 키**이며 장면 초기화·새 에피소드 시작.
  수집 시 이전/다음 시도는 demo_0, demo_1 등으로 나뉜다.
- **ESC로 종료 후 `[Dataset export] READY`까지 터미널을 유지해야 한다.** 창이 닫힌 것만으로 저장 완료가 아니다.
- 기본 수집은 deferred: 조종 중 물리 상태·입력을 기록하고, 종료 후 별도 headless 프로세스에서
  같은 상태·카메라·시각으로 RGB-D와 HDF5 생성. `capture.json`의 status=ready, complete=true 확인.
- `--training-record 1`은 원본 전체 기록도 자동 활성화한다.
  `--full-record 1` 단독은 재생·검증용 로봇/옷/입력 상태만 저장하며 학습 RGB-D/HDF5는 만들지 않는다.
  후자는 README 빠른 실행표에서 제외했고 기능·상세 설명은 유지한다.
- 별도 PHYRC_ACCEPT_EULA 입력 없이 실행된다. NVIDIA 이용 조건 안내는 SETUP 문서에 있다.
- replay는 저장된 3D 물리 상태를 복원하는 방식이다. MP4 재생이나 동일 PhysX 계산의 재실행이라고 설명하지 말 것.

## 정책 인터페이스와 평가

- 정책 20Hz, 제어 60Hz, 물리 240Hz. 한 action을 0.05시뮬레이션초=제어3회=물리12tick 유지한다.
- action `(2,9)` = 로봇2대 × 로봇당9명령; 저장 시 `(N,18)`.
  로봇당 순서는 전진/왼쪽/베이스회전/리프트/팔신장/손목yaw/pitch/roll/그리퍼.
  정규화 범위[-1,1]. 그리퍼 +1닫기, −1열기, 0이전 의도 유지. 관절 측정값과 명령을 구분할 것.
- 허용 관측: 외부1 + 양 로봇 손목·상단4, 총5카메라의 RGB/depth/유효mask와 공개 로봇 상태.
  상세 shape·단위·열 순서는 `config/policy_interface.json` 및 `docs/competition/DATA.md`.
- 학습에는 policy.hdf5의 허용 관측·액션만 사용. 원본 옷 정점/물체 정답 위치/내부 파지·평가값은
  재생·검증용이며 정책 입력·행동 라벨 생성에 사용 금지. RULES의 주최측 검증·적발 시 심사 제외 안내 유지.
- 총50점: 집기5, 첫소매5, 반대어깨5, 두번째소매5, Overall30(상박좌5/우5/목10/브이넥10).
  손은 서로 다른 소매 구멍 밖으로 나와야 하고 상박은 해당 소매 덮임 여부로 채점. 하박·몸통 점수 없음.
  목 통과는 즉시 판정; 브이넥 및 동시 착의 성공 확인은0.5시뮬레이션초 유지. 누적50점과 착의성공은 구분.
- points/s 규칙 `pickup-excluded-last-award-v9`: 집기 제외 점수 / (마지막 착의 득점−최초 옷·마네킹 충돌 메시 접촉).
  시뮬레이션 시간 기준이고 마지막 득점 뒤 대기로 분모가 늘지 않는다. 무접촉 시도도 집계 중단 없이 결과에 포함.

## 검증·파일 위치·주의할 한계

- 기존 source43개 Python 파일과 `.runtime/DexGarmentLab` 파일이 인계 시점에 모두 일치함을 재확인했다.
  실행 소스를 수정할 때는 필요한 파일을 runtime에도 반영한다.
  **기존 작업 폴더에서 무심코 prepare를 재실행하지 말 것:** runtime 자산을 재생성한다.
- 같은 PC에서 GitHub 새 clone, 별도 Docker build, 새 자산 다운로드·prepare·실제 smoke·GUI 조종·ESC·자동 HDF5 변환을 검증했다.
  1에피소드/9학습구간/108물리tick 누락 없음, 5RGB-D 및 로봇상태·액션·카메라/viewport 시각 대조 PASS.
  첫 빈 캐시 smoke는 약342초; 오래 걸리는 초기화를 곧바로 오류라고 단정하지 말 것.
- smoke의 과거 기대값을 현재 배포 상태(정점15878/삼각형31344/solver64)에 맞게 수정했다.
  테스트를 맞추려고 환경 물성을 바꾼 것이 아니다.
- 근거: `docs/competition/VERIFICATION.md`, 로컬 `output/competition_publish/clean_*`.
  새 설치 테스트 폴더 `/tmp/phyrc-clean-AC5RlluD/repo`는 임시 경로이므로 영구 자료로 의존하지 말 것.
- 참가자 예제는 `20260913T153054_259148Z_0e4bae`: 1,142구간/57.1sim초, 집기5점의 부분시연.
  원본: `output/full_teleop/<ID>/`, `output/policy_datasets/<ID>/`.
  열람: `output/dataset_reviews/<ID>/index.html`; 공개 예제: `docs/examples/teleop/`, `docs/videos/`.
  이 CSV와 MP4 8개는 같은 기록에서 생성된 것이 확인되었다.
- 상단 `Back_Front_32x.mp4`와 위 기록이 같은 실행인지 연결 ID로 확인되지는 않았다.
  사용자는 같은 실행인 것 같다고 지적했고, 근거 없던 “서로 다른 실행” 문구는 삭제했다. 재삽입하지 말 것.
- 다른 PC/새 OS 설치·장시간 안정성·완전 착의 정책 학습 성공은 이번 검증 범위 밖이다.
  CPU 평가 회귀 및 앞선 GPU 검증의 상세 결과는 아래 이력과 보존 로그를 참고한다.
...
사용자는 현재 대회용 저장소 정리와 인계 문서의 반영·게시를 요청했다.
아래 작업 이력의 “커밋/푸시 없음·금지”는 각 작업 당시 상태이며, 이후 승인된 게시가 완료되었다.
이를 향후 모든 새 작업에 대한 자동 게시의 상시 승인으로 해석하지 않는다.
또한 사용자는 replay 배속을 더 이상 수정하지 말라고 요청했다.

# 작업 이력 읽는 법

**이 아래는 각 작업 당시의 기록이다.** 과거 브랜치명·HEAD·평가 규칙·미커밋 여부는 현재 상태와 다를 수 있다.
현재 상태와 충돌하면 맨 위의 **현재 인계 요약**과 **사용자 지속 지침**을 우선한다.
과거의 실시간 시간 채점, 별도 competition 브랜치, 이전 목 유지 시간 규칙 등을 현재 설정으로 되돌리지 않는다.

# 시연 영상 출처 단정 문구 삭제 — 2026-09-14

- 사용자가 상단32배속 영상과 학습 예제가 같은 실행인 것 같다고 지적.
  README의 “서로 다른 실행입니다” 문구를 삭제. 다른 실행이라는 확정 근거가 없었음.
- 예제 CSV 값(원본과 줄바꿈 차이는 있음) 및 MP4 8개 SHA가 모두
  output/dataset_reviews/20260913T153054_259148Z_0e4bae 산출물과 일치함을 확인.
  따라서 학습 데이터의 CSV·5카메라·viewport 예제는 같은 기록에서 생성됨.
- 사용자 제공 Back_Front_32x.mp4에는 원본 recording ID 연결 메타데이터가 없음.
  여러 프레임도 비교했지만 같은 실행/다른 실행 중 어느 쪽도 확정하지 않음.
  파일 길이·편집 시각·카메라 구도 차이만으로 다른 실행이라고 다시 기재하지 말 것.
- 비교용 프레임은 output/competition_publish/video_source_check/에 있음.

# main 단일 대회용 브랜치로 통합 — 2026-09-14

- 사용자 명시 요청: main 하나만 사용하고 main을 대회용 GitHub로 운영.
  competition의 fea7dc7까지 main에 fast-forward로 통합한 뒤 안내를 수정.
  main 게시 확인 후 원격/로컬 competition 브랜치를 삭제; 모든 커밋 이력은 main에 보존.
  앞으로 참가자 문서와 실행 기능 작업의 기준 브랜치는 main. 과거 competition 전용 지시는 이 요청으로 대체됨.
- 사용자 문체 선호: “충돌 메시 표시는 기본적으로 꺼져 있습니다” 같은 불필요한 설명을 넣지 않음.
  README에서 해당 문구 및 과거 EULA 환경 변수 제거 설명을 삭제. 이용 조건 링크는 유지.
- README 실행표의 원본 기록 전용 --full-record 행을 제거하고 학습 수집 --training-record 중심으로 안내.
  DATA에는 --full-record가 로봇/옷/입력 등 재생·검증용 물리 상태만 저장하며
  RGB-D/학습 HDF5는 생성하지 않는다는 설명 추가. 기능 자체는 유지.
  run.sh는 help 문구만 명확히 했으며 옵션/동작/물리 설정은 변경하지 않음.
- P는 GUI 버튼이 아닌 키보드 P 키임을 README/DATA/EVALUATION에 명시.
  viewport 클릭 후 P로 장면 초기화, 한 번의 시도=에피소드, demo_0→demo_1 예시 추가.
- README/SETUP의 clone 명령은 기본 main을 사용. 검증 결과 문서는 검증 코드가 main에 통합되었음을 안내.
- 검증: Markdown 로컬 링크, bash -n 및 ./run.sh help, main이 이전 competition 이력을 포함하는지 확인.
  실행 환경 소스/자산 변경이 없으므로 앞 절의 실제 GPU 설치·수집 검증 결과가 그대로 적용됨.
